...
const ButtonEditTextSize = (props: TProps) => {
	...
	const fontSizeForm = formCore.form_title_size || formCore.form_setting_default.form_title_size_default;
		...
		if (typeEdit === "Input") {
			newFormEdit.form_inputs = newFormEdit.form_inputs.map((ip) => {
				if (ip._id === inputItem?._id) {
					ip.setting = { ...ip.setting, input_size: size };
					return ip;
				}
	...
	const titleCoreSize =
		typeEdit === "Form"
			? formCore.form_title_size || formCore.form_setting_default.form_title_size_default
		...
		>
			...
			<div className="w-[6rem] flex items-center justify-center">
				<input
					...
					value={titleCoreSize}
					className="w-[5rem] h-[3.6rem] text-center  bg-[#464646] text-[#ccc] rounded-lg outline-none"
				/>
			</div>
