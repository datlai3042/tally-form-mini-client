...
import InputPhoneAnswerTitle from "./InputPhoneAnswerTitle";
...
const InputPhoneAnswer = (props: TProps) => {
	...
	const onBlur = (e: React.ChangeEvent<HTMLDivElement>) => {
		...
		if (write) {
			...
			if (phone.toString().length > 0) {
				...
				const superValidate = superPhoneValidate(phone.toString(), setting);
				...
				if (_next) {
					...
					//xét data global
					setDataInputGlobal(setFormAnswer, inputItem._id!, phone.toString());
					return;
				}
				//fail validate
				else {
	...
	return (
		<InputAnswerWrapper>
			...
			>
				<InputPhoneAnswerTitle inputItem={inputItem} formCore={formCore} />
