...
import InputOptionMultipleTitle from "./InputOptionMultipleTitle";
...
const InputOptionMultipleAnswer = (props: TProps) => {
	...
	return (
		<InputAnswerWrapper>
			...
			>
				<InputOptionMultipleTitle formCore={formCore} inputItem={inputItem} />
									...
									<div
										...
										onClick={() => {
											...
											if (
												...
												choose.input_value.includes(op.option_value)
											) {
											...
											if (choose.input_value.includes(op.option_value)) {
												const newArrayOption = choose.input_value.filter(
													(option) => option !== op.option_value
												);
												console.log({ newArrayOption, choose: choose.input_value });
												setDataInputGlobal(setFormAnswer, inputItem._id!, newArrayOption);
				...
				<p className="text-[1.4rem]">
					Đã chọn:{" "}
					<span className="ml-[.4rem] border-b-[.2rem] border-gray-400">{choose.input_value.join(", ")}</span>
				</p>
